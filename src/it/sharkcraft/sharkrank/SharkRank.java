...
public void onEnable()
  {
    ...
    saveConfig();
    Bukkit.getServer().getPluginManager().registerEvents(new KillsListener(this), this);
  }
  
  public void onDisable()
  {
    Bukkit.getServer().getConsoleSender().sendMessage(ChatColor.GRAY + "[" + ChatColor.RED + ChatColor.BOLD + "!" + 
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args)
  {
    ...
    if (cmd.getName().equalsIgnoreCase("shreload"))
    {
      saveConfig();
      reloadConfig();
      p.sendMessage("§8[§c§l!§8] §9SharkRank>  §7Reload Complete.");
    }
    if (cmd.getName().equalsIgnoreCase("shupdate")) {
      Commands.rankUp(p);
    }
    return false;
  }
